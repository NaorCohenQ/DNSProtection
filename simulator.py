...
class DNSProtectionLayer:
    ...
    def process_request(self, packet):
        ...
        if nxdomain:
            ...
            if self.denied[ip]>self.rhhh.getAmount(ip)+2*self.threshold:
...
attack_traffic = generate_attack_traffic(subnets, num_requests=10000, start_timestamp=first_timestamp)
...
# legitimate_requests = sum(1 for _, _, orig in combined_traffic if orig == 1)
legitimate_requests = len(real_traffic)
...
for packet in combined_traffic:
   ...
   if result == "Blocked":
        blocked_count += 1
        legit_blocked = legit_blocked + packet[is_orig]

print(f"Blocked Requests: {blocked_count}")
print(f"Total Requests: {total_requests}")
print(f"Legitimate Requests: {legitimate_requests}")
print(f"Legitimate Requests Blocked: {legit_blocked}")
print(f"Attack Requests Blocked: {blocked_count-legit_blocked}")
